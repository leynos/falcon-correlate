...
class TestMergeContextvarsLimitation:
    ...
    def test_merge_contextvars_ignores_correlation_id_var(
        self,
        isolated_context: cabc.Callable[[cabc.Callable[[], None]], None],
    ) -> None:
    ...
    def test_merge_contextvars_ignores_user_id_var(
        self,
        isolated_context: cabc.Callable[[cabc.Callable[[], None]], None],
    ) -> None:
...
class TestCustomProcessorApproach:
    ...
    def test_custom_processor_injects_ids(
        self,
        isolated_context: cabc.Callable[[cabc.Callable[[], None]], None],
    ) -> None:
        ...
        assert captured[0]["user_id"] == "proc-uid-001", (
            f"expected 'proc-uid-001', got {captured[0].get('user_id')!r}"
        )

    def test_custom_processor_uses_placeholder_when_unset(
        self,
        isolated_context: cabc.Callable[[cabc.Callable[[], None]], None],
    ) -> None:
        ...
        assert captured[0]["user_id"] == "-", (
            f"expected '-', got {captured[0].get('user_id')!r}"
        )
