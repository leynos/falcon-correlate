...
The caller passes no `with:` inputs at all, so the shared workflow's defaults
apply throughout: `paths` defaults to `src/` and `module-prefix-strip`
defaults to `src/`, both of which already match this repository's src-layout
package at `src/falcon_correlate/`. A contract test asserts the `with:` block
stays absent, so a future change that starts overriding those defaults must
...
introducing an undocumented input — rather than letting the breakage surface
only in a scheduled run. This test module has no `skipif` guard: it runs
under the ordinary `pytest` collection, and `[tool.mutmut]`'s
`pytest_add_cli_args_test_selection` keeps it (and the other workflow and
documentation audits) out of mutmut's own test selection, since mutmut's
`mutants/` working copy omits `.github/` and the tests would kill no mutants
there. Run it locally with:
...
- the `uses:` reference targets `mutation-mutmut.yml` pinned to a full commit
  SHA;
- the job declares no `with:` block, so the shared defaults apply;
